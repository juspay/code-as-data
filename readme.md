...
1. Clone the repository:
   ...
   ```bash
   git clone ssh://git@ssh.bitbucket.juspay.net/~eswar.tadiparth_juspay.in/code-as-data.git
   cd code-as-data
   ```
...
```bash
python scripts/import_dumps.py /path/to/code/dumps --clear
```
...
pattern_matches = query_service.find_code_patterns(
    pattern_code,
    min_matches=3  # Minimum number of matching lines
)
```

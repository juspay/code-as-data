...
def generate_code_embedding(code_text):
    ...
    try:
        response = openai_client.embeddings.create(
            model=os.getenv(
                "AZURE_OPENAI_CODE_EMBEDDING_DEPLOYMENT", "code-embedding-ada-002"
            ),
            input=code_text,
        )
        return response.data[0].embedding
    ...
    try:
        response = openai_client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a code analysis assistant that provides detailed technical analysis.",
                },
...
def calculate_complexity(function_data):
    ...
    if function_data.get("line_number_start") and function_data.get("line_number_end"):
        line_count = (
            function_data["line_number_end"] - function_data["line_number_start"]
        )
        ...
        for module in tqdm(modules, desc="Processing modules"):
            ...
            for function in functions:
                ...
                for caller in callers:
                    ...
                    called_by.append(
                        {"id": caller_id, "name": caller_name, "module": caller_module}
                    )
            ...
            for import_obj in imports:
                import_id, import_module, import_package, qualified_style, is_hiding = (
                    import_obj
                )
...
def process_data_for_qdrant(extracted_data):
    """Process extracted data and store in Qdrant."""
    ...
    for module_data in tqdm(extracted_data, desc="Processing data for Qdrant"):
        ...
        for function in module_data["content"]["functions"]:
            ...
            function_doc = {
                ...
                "vectors": {
                    ...
                    "code_vector": generate_code_embedding(
                        function.get("raw_string", "")
                    ),
                },
            }
            ...
            if (
                function["metrics"]["complexity"] > 5
                or function["metrics"]["dependency_count"] > 10
            ):
                # Analyze performance
                if function.get("raw_string"):
                    performance_analysis = analyze_code_with_llm(
                        function["raw_string"], "performance"
                    )
        ...
        combined_code = "\n".join(
            [
                f.get("raw_string", "")
                for f in module_data["content"]["functions"][:5]
                if f.get("raw_string")
            ]
        )
        if combined_code:
            ...
            domain_doc = {
                ...
                "vectors": {
                    "semantic_vector": generate_semantic_embedding(
                        domain_knowledge_text
                    )
                },
            }
        ...
        combined_code = "\n".join(
            [
                f.get("raw_string", "")
                for f in module_data["content"]["functions"][:3]
                if f.get("raw_string")
            ]
        )
        if combined_code:
            ...
            review_doc = {
                ...
                "payload": {
                    ...
                    "module": module_data["name"],
                    "issue_count": code_quality_analysis.count("issue")
                    + code_quality_analysis.count("problem"),
                },
                "vectors": {
                    "semantic_vector": generate_semantic_embedding(
                        code_quality_analysis
    ...
    for i in tqdm(
        range(0, len(code_elements), batch_size), desc="Uploading code elements"
    ):
        ...
        qdrant_client.upsert(
            ...
            points=[
                qdrant_models.PointStruct(
                    id=doc["id"], payload=doc["payload"], vectors=doc["vectors"]
                )
                for doc in batch
            ],
        )
    ...
    for i in tqdm(
        range(0, len(domain_knowledge), batch_size), desc="Uploading domain knowledge"
    ):
